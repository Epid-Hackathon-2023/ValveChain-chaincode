...
class ValveMaintenanceContract extends Contract {
    ...
    async createVanne(ctx, vanne_id, name, description, position_c, position_a, temp_relevee_amont, temp_relevee_aval, temp_attendue) {
        const vanne = {
            vanne_id: vanne_id,
            name: name,
    ...
    async getVanneById(ctx, vanne_id) {
        ...
        if (!vanneBuffer || vanneBuffer.length === 0) {
            throw new Error("La vanne ${vanne_id} n'existe pas");
        }
